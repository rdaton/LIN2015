...
static int remove (char* valor,struct list_head* list){
	...
	if(strcmp(item->data,valor)==0){
		trace_printk(KERN_INFO "el valor que va a eliminar es %s\n",valor);
		list_del(cur_node);
		if (item->data=NULL)
			vfree(item->data);
		vfree(item);
...
static ssize_t modlist_write(struct file *filp, const char __user *buf, size_t len, loff_t *off) {
	...
	unBuffer[len]='\0';
		...
		trace_printk("add opt 306"); 
	  if(sscanf(unBuffer,"add %s\n",&r)==1)
	  	{	
	  		add(r);
	...
	else if(sscanf(unBuffer,"remove %s\n",&r)==1)
	 {
	  		remove(r,&modlist);
	...
	else if(sscanf(unBuffer,"push %s\n",&r)==1) 
		{
	 
		
	  		push(r);
	  		trace_printk("intentando a push: %s\n",r);
	  		print_list(&modlist);
	  }
	  	...
	  	trace_printk("add norm 332");
	  if(sscanf(unBuffer,"add %i\n",&r)==1)
